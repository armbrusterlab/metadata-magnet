...
transform_metadata <- function(metadata_file, outname) {
  ...
  df2 <- df |> pivot_wider(names_from="Assembly.BioSample.Attribute.Name", values_from="Assembly.BioSample.Attribute.Value")
  ...
  titles <- df2 |> select(all_of(title_cols)) |> mutate(across(title_cols, ~ str_extract(.x, "(?<=from ).*") |> str_replace_na(replacement = "")))
  ...
  df_useful <- df2 |> select(any_of(c("Assembly.BioSample.Isolation.source", "Assembly.BioSample.Host.disease", "Isolation Site"))) |> mutate(across(everything(), ~ str_replace_na(.x, replacement = "")))
  ...
  # save to file
  df2 |> write.table(file=outname, quote=FALSE, sep='\t', row.names=FALSE) # rownames false so that it doesn't add a 1, 2, 3... to the left of the table
}
